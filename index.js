...
async function main() {
    ...
    const { queries, interval, resolution: resolutionArgument, } = await y
        .usage('$0 [-q <queries...>] [-i <hours> <minute> <seconds>] [-r <width> <height>]')
        .option('queries', {
        alias: 'q',
        array: true,
        description: 'List of query terms to search for wallpapers under, <queries...> e.g. space nature',
        default: '',
        ...
        alias: 'i',
        type: 'array',
        description: 'Time interval between updating wallpaper, <hours> <minutes> <seconds> e.g. 0 30 0',
        ...
        coerce(values) {
            ...
            validateNonNegativeIntegerArray(values, 3, 'Interval must consist of 3 non-negative integers in the format <hours> <minutes> <seconds> e.g. 0 30 0');
        ...
        alias: 'r',
        type: 'array',
        description: 'Minimum resolution of the wallpaper, <width> <height> e.g. 1920 1080',
        defaultDescription: "fetch your screen's resolution",
        coerce(values) {
            // ensure 2 non-negative integers are provided for width and height
            validateNonNegativeIntegerArray(values, 2, 'Resolution must consist of 2 non-negative integers in the format <width> <height> e.g. 1920 1080');
...
async function fetchAndSetWallpaper(queries, resolution) {
    ...
    const wallpaperPath = join(import.meta.dirname, `wallpaper${extname(imageUrl)}`);
